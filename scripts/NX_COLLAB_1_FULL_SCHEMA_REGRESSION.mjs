...
const full="['0001_schema.sql','0002_seed.sql','0003_inquiry_locale.sql','0004_clients_core.sql','0005_clients_hardening.sql','0006_clients_code_alignment.sql','0007_client_projects.sql','0008_client_project_execution.sql','0009_client_project_portal_support.sql','0010_client_project_collaboration.sql']";
...
for(const rel of workerTests){
  const src=path.join(root,rel),dir=path.dirname(src),tmp=path.join(dir,`.nx-collab1-${path.basename(rel)}`);
...
console.log('NX_COLLAB_1_FULL_SCHEMA_REGRESSION_PASS');
