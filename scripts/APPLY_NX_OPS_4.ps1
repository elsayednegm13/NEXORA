...
$ExpectedMigration = '0009_client_project_portal_support.sql'

$NodeExe = $env:NEXORA_NODE_EXE
if ([string]::IsNullOrWhiteSpace($NodeExe) -or -not (Test-Path -LiteralPath $NodeExe)) {
    throw 'Verified Node runtime is missing. Run .\scripts\RUN_NX_OPS_4_RELEASE.ps1 instead of APPLY_NX_OPS_4.ps1 directly.'
}
...
if ([string]::IsNullOrWhiteSpace($WranglerJs) -or -not (Test-Path -LiteralPath $WranglerJs)) {
    throw 'Verified Wrangler runtime is missing. Run .\scripts\RUN_NX_OPS_4_RELEASE.ps1 instead of APPLY_NX_OPS_4.ps1 directly.'
}
...
Write-Host 'NEXORA NX-DATA-4 + NX-OPS-4 - Portal + Support Foundation Release Gate' -ForegroundColor Cyan
...
& $NodeExe .\scripts\NX_OPS_4_CHECK.mjs
if($LASTEXITCODE -ne 0){throw 'NX-OPS-4 engineering gate failed. Remote actions are blocked.'}
...
if($ApplyDataRemote){
    if($pending.Count -ne 1 -or $pending[0] -ne $ExpectedMigration){
        throw "NX-DATA-4 apply requires exactly one pending migration: $ExpectedMigration. Found: $($pending -join ', ')"
    }
    Write-Host 'Reading protected production counts before NX-DATA-4 (read-only, per-table)...' -ForegroundColor Cyan
    ...
    if($LASTEXITCODE -ne 0){throw 'Current live runtime check failed before NX-DATA-4.'}
    ...
    if($WranglerExitCode -ne 0){throw 'NX-DATA-4 remote migration apply failed.'}

    Write-Host 'Verifying no pending migrations remain...' -ForegroundColor Cyan
    $afterMigration=Get-MigrationState
    if(@($afterMigration.Pending).Count -ne 0){throw "Unexpected pending migrations remain after NX-DATA-4: $(@($afterMigration.Pending) -join ', ')"}

    Write-Host 'Verifying Portal + Support tables exist and start empty...' -ForegroundColor Cyan
    foreach($table in $SupportTables){
        $count=Get-RemoteCount -Sql "SELECT COUNT(*) AS rows_count FROM $table;" -Context "post-data table: $table"
        if($count -ne 0){throw "New Portal/Support table is not empty: $table=$count"}
    }
    Write-Host 'PASS Portal + Support tables exist and start empty.' -ForegroundColor Green
    ...
    if($taskVisibleColumn -ne 1 -or $milestoneVisibleColumn -ne 1){throw 'Client visibility columns are missing after NX-DATA-4.'}
    ...
    Write-Host 'Re-reading protected production counts after NX-DATA-4...' -ForegroundColor Cyan
    ...
    if($beforeSig -ne $afterSig){throw "Protected production counts changed during schema-only NX-DATA-4. Before=$beforeSig After=$afterSig"}
    ...
    if($clientCodeMismatchesAfter -ne 0){throw 'Client Code invariant changed during NX-DATA-4.'}

    Write-Host 'Running live runtime smoke after migration...' -ForegroundColor Cyan
    .\scripts\RUNTIME_CHECK.ps1
    if($LASTEXITCODE -ne 0){throw 'Runtime check failed after NX-DATA-4 migration.'}

    Write-Host ''
    Write-Host 'NX-DATA-4 REMOTE APPLY COMPLETE.' -ForegroundColor Green
    Write-Host 'No Worker/Admin deployment or Portal/Support business-row seed was performed.' -ForegroundColor Green
    Write-Host 'Next owner gate: run read-only preflight again, then approve -DeployRemote.' -ForegroundColor Yellow
    exit 0
}

if($DeployRemote){
    if($pending.Count -ne 0){throw "NX-OPS-4 deploy requires no pending migrations. Found: $($pending -join ', ')"}

    Write-Host 'Verifying Portal + Support tables are available before code deployment...' -ForegroundColor Cyan
    foreach($table in $SupportTables){[void](Get-RemoteCount -Sql "SELECT COUNT(*) AS rows_count FROM $table;" -Context "pre-deploy table: $table")}
    Write-Host 'PASS Portal + Support table availability.' -ForegroundColor Green

    $allTables=@($BaseCountTables+$ExecutionTables+$SupportTables)
    Write-Host 'Reading production counts before NX-OPS-4 deployment (read-only, per-table)...' -ForegroundColor Cyan
    ...
    .\scripts\RUNTIME_CHECK.ps1
    if($LASTEXITCODE -ne 0){throw 'Current live runtime check failed before NX-OPS-4 deployment.'}

    Write-Host 'Deploying NX-OPS-4 Worker + Admin assets...' -ForegroundColor Cyan
    ...
    .\scripts\RUNTIME_CHECK.ps1
    if($LASTEXITCODE -ne 0){throw 'Runtime check failed after NX-OPS-4 deployment.'}
    ...
    $afterMigration=Get-MigrationState
    if(@($afterMigration.Pending).Count -ne 0){throw "Unexpected pending migration appeared after NX-OPS-4 deployment: $(@($afterMigration.Pending) -join ', ')"}
    ...
    if($clientCodeMismatchesAfter -ne 0){throw 'Client Code invariant changed during NX-OPS-4 deployment.'}
    ...
    Write-Host ''
    Write-Host 'NX-OPS-4 REMOTE DEPLOY COMPLETE.' -ForegroundColor Green
    Write-Host 'Real-browser acceptance remains required: Admin Tickets, Client Access link, client-visible Milestones/Tasks, Portal ticket submission/tracking, AR/EN, Dark/Light and responsive layouts.' -ForegroundColor Yellow
    exit 0
}

# Read-only preflight is state-aware: before 0009 it prepares Data Gate; after 0009 it prepares Code Gate.
if($pending.Count -eq 1 -and $pending[0] -eq $ExpectedMigration){
    ...
    Write-Host '.\scripts\RUN_NX_OPS_4_RELEASE.ps1 -ApplyDataRemote' -ForegroundColor White
    exit 0
}

if($pending.Count -eq 0){
    Write-Host 'Verifying Portal + Support tables are available (read-only)...' -ForegroundColor Cyan
    foreach($table in $SupportTables){[void](Get-RemoteCount -Sql "SELECT COUNT(*) AS rows_count FROM $table;" -Context "preflight table: $table")}
    [void](Get-RemoteCountSnapshot -Phase 'preflight-deploy' -Tables @($BaseCountTables+$ExecutionTables+$SupportTables))
    Write-Host 'Running current live runtime smoke...' -ForegroundColor Cyan
    .\scripts\RUNTIME_CHECK.ps1
    if($LASTEXITCODE -ne 0){throw 'Current live runtime check failed.'}
    Write-Host ''
    Write-Host 'PRE-FLIGHT PASS - CODE GATE. 0009 is already applied and no migration is pending.' -ForegroundColor Green
    Write-Host 'No Worker deployment, migration, Secret change, or business-data write was performed.' -ForegroundColor Green
    Write-Host 'After owner review run:' -ForegroundColor Yellow
    Write-Host '.\scripts\RUN_NX_OPS_4_RELEASE.ps1 -DeployRemote' -ForegroundColor White
    exit 0
}
