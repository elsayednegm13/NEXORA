...
$ManifestPath = Join-Path $PSScriptRoot 'NX_OPS_2_RELEASE_MANIFEST.json'
...
Write-Host 'NEXORA NX-OPS-2 CLEAN RELEASE RUNNER R5' -ForegroundColor Cyan
Write-Host 'R5 keeps the pinned runtime and validates Wrangler D1 JSON with a dedicated Node parser, avoiding PowerShell response-shape ambiguity.' -ForegroundColor DarkGray

if (-not (Test-Path -LiteralPath $ManifestPath)) { throw 'Release manifest is missing.' }
$manifest = Get-Content -Raw -Encoding UTF8 $ManifestPath | ConvertFrom-Json
if ($manifest.release -ne 'NX-OPS-2-R5') { throw 'Unexpected release manifest identity.' }
...
# Pin the Windows Node runtime. We intentionally do not use the machine-wide Node 24 runtime.
...
if (Test-Path -LiteralPath $NodeExe) {
    ...
    if ($LASTEXITCODE -eq 0 -and $detectedNode -eq $ExpectedNodeVersion) { $nodeValid = $true }
}
...
if (-not $nodeValid) {
    ...
    try {
        ...
        if ($archiveHash -ne $ExpectedNodeArchiveSha256) {
            throw "Node archive SHA-256 mismatch. Expected $ExpectedNodeArchiveSha256, got $archiveHash"
        }
...
if (-not (Test-Path -LiteralPath $NodeExe) -or -not (Test-Path -LiteralPath $NpmCmd)) {
    throw 'Portable Node.js runtime was not installed correctly.'
}
$detectedNode = (& $NodeExe --version 2>$null | Out-String).Trim()
if ($LASTEXITCODE -ne 0 -or $detectedNode -ne $ExpectedNodeVersion) {
    throw "Portable Node.js verification failed. Required $ExpectedNodeVersion, detected $detectedNode"
}
Write-Host "PASS isolated Node runtime: $ExpectedNodeVersion" -ForegroundColor Green

# Install Wrangler with Node 22's own npm into an isolated cache.
...
if (Test-Path -LiteralPath $WranglerJs) {
    ...
    if ($LASTEXITCODE -eq 0 -and $versionText -match [regex]::Escape($ExpectedWranglerVersion)) { $wranglerValid = $true }
}
if (-not $wranglerValid) {
    ...
    try {
        & $NpmCmd install --no-save --no-package-lock --no-audit --no-fund "wrangler@$ExpectedWranglerVersion"
        if ($LASTEXITCODE -ne 0) { throw 'Pinned Wrangler installation failed.' }
    } finally { Pop-Location }
}
if (-not (Test-Path -LiteralPath $WranglerJs)) { throw 'Pinned Wrangler runtime was not installed correctly.' }
$versionText = (& $NodeExe $WranglerJs --version 2>$null | Out-String).Trim()
if ($LASTEXITCODE -ne 0 -or $versionText -notmatch [regex]::Escape($ExpectedWranglerVersion)) {
    throw "Pinned Wrangler verification failed. Detected: $versionText"
}
Write-Host "PASS isolated Wrangler runtime: $ExpectedWranglerVersion" -ForegroundColor Green

$StageRoot = Join-Path $env:TEMP ("NEXORA_NX_OPS_2_R5_" + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    foreach ($entry in $manifest.files) {
        $src = Join-Path $SourceRoot $entry.path
        $dst = Join-Path $StageRoot $entry.path
        $dstDir = Split-Path -Parent $dst
        if (-not (Test-Path -LiteralPath $dstDir)) { New-Item -ItemType Directory -Path $dstDir -Force | Out-Null }
        Copy-Item -LiteralPath $src -Destination $dst -Force
    }
    Copy-Item -LiteralPath $ManifestPath -Destination (Join-Path $StageRoot 'scripts\NX_OPS_2_RELEASE_MANIFEST.json') -Force

    $env:NEXORA_NODE_EXE = $NodeExe
    $env:NEXORA_WRANGLER_JS = $WranglerJs
    Push-Location $StageRoot
    try {
        if ($DeployRemote) {
            & .\scripts\APPLY_NX_OPS_2.ps1 -DeployRemote
        } else {
            & .\scripts\APPLY_NX_OPS_2.ps1
        }
        if ($LASTEXITCODE -ne 0) { throw "NX-OPS-2 release execution failed with exit code $LASTEXITCODE." }
    } finally { Pop-Location }
    $success = $true
} finally {
    ...
    if ($success) {
        ...
        Write-Host "Diagnostic stage preserved at: $StageRoot" -ForegroundColor Yellow
    }
}
