...
for(const [rel,expected] of Object.entries(manifest)){
  // wrangler.jsonc is the single approved deployment-state reconciliation file.
  // Every other pre-existing NX-DATA-1 file must remain byte-identical.
...
if(protectedOk){
  const protectedCount=Object.keys(manifest).filter(x=>x!=='wrangler.jsonc').length;
  pass(`NX-DATA-1 protected files byte-identical except approved live config reconciliation (${protectedCount})`);
...
// 1b) Canonical Cloudflare live configuration must exactly target the verified existing Worker/D1.
let liveCfgOk=true;
let liveCfg;
try{liveCfg=JSON.parse(read('wrangler.jsonc'));}catch(e){fail(`wrangler.jsonc parse failed: ${e.message}`);liveCfgOk=false;}
if(liveCfgOk){
  const expectedVars={
    MAX_JSON_BYTES:'262144',
    INQUIRY_RATE_LIMIT:'5',
    INQUIRY_RATE_WINDOW_SECONDS:'3600',
    ADMIN_SESSION_TIMEOUT_SECONDS:'28800',
    ADMIN_LOGIN_RATE_LIMIT:'8',
    ADMIN_LOGIN_RATE_WINDOW_SECONDS:'900'
  };
  const checks=[
    [liveCfg.name==='nexoratechnologies','Worker name'],
    [liveCfg.main==='./src/worker.js','Worker entry'],
    [liveCfg.compatibility_date==='2026-09-06','compatibility_date'],
    [liveCfg.workers_dev===true,'workers_dev'],
    [liveCfg.assets?.directory==='./public','assets.directory'],
    [liveCfg.assets?.binding==='ASSETS','ASSETS binding'],
    [JSON.stringify(liveCfg.assets?.run_worker_first)===JSON.stringify(['/api/*']),'assets run_worker_first'],
    [JSON.stringify(liveCfg.vars)===JSON.stringify(expectedVars),'plain-text vars'],
    [liveCfg.observability?.enabled===true,'observability'],
    [Array.isArray(liveCfg.d1_databases)&&liveCfg.d1_databases.length===1,'single D1 binding']
  ];
  for(const [ok,label] of checks)if(!ok){fail(`canonical live config mismatch: ${label}`);liveCfgOk=false;}
  const db=liveCfg.d1_databases?.[0];
  if(!db||db.binding!=='DB'||db.database_name!=='nexora-db'||db.database_id!=='71c3bf88-b71d-465b-9032-5251a11fde64'){
    fail('canonical live D1 identity mismatch');liveCfgOk=false;
  }
  if(liveCfgOk)pass('canonical live config exact: nexoratechnologies / DB -> nexora-db / verified UUID');
}
...
if(process.exitCode){console.error('\nNX-OPS-1 LIVE CHECK: FAIL');process.exit(process.exitCode);}else console.log('\nNX-OPS-1 LIVE CHECK: PASS');
