...
$ExpectedCssMarker = 'NX-COLLAB-1 UI R4.1'
$D1ReadMaxAttempts = 4
...
if ([string]::IsNullOrWhiteSpace($NodeExe) -or -not (Test-Path -LiteralPath $NodeExe)) {
    throw 'Verified Node runtime is missing. Run .\scripts\RUN_NX_COLLAB_1_UI_R4_1_RELEASE.ps1 instead of APPLY_NX_COLLAB_1_UI_R4_1.ps1 directly.'
}
...
if ([string]::IsNullOrWhiteSpace($WranglerJs) -or -not (Test-Path -LiteralPath $WranglerJs)) {
    throw 'Verified Wrangler runtime is missing. Run .\scripts\RUN_NX_COLLAB_1_UI_R4_1_RELEASE.ps1 instead of APPLY_NX_COLLAB_1_UI_R4_1.ps1 directly.'
}
...
function Assert-LiveCssMarker {
    ...
    try{$res=Invoke-WebRequest -Uri $Url -UseBasicParsing -TimeoutSec 30}catch{throw 'Admin CSS runtime request failed after R4.1 deployment.'}
    if($res.StatusCode -ne 200){throw 'Admin CSS runtime status is not 200 after R4.1 deployment.'}
    if($res.Content -notmatch [regex]::Escape($ExpectedCssMarker)){throw 'Admin CSS does not serve the R4.1 containment marker after deployment.'}
    Write-Host 'PASS R4.1 containment CSS marker served: Admin UI' -ForegroundColor Green
}
...
Write-Host 'NEXORA NX-COLLAB-1 UI R4.1 - Message/Composer Containment Correction' -ForegroundColor Cyan
...
Write-Host 'Running R4.1 engineering + real-DOM browser containment gate before any Cloudflare action...' -ForegroundColor Cyan
& $NodeExe (Join-Path $PSScriptRoot 'NX_COLLAB_1_UI_R4_1_CHECK.mjs')
if($LASTEXITCODE -ne 0){throw 'R4.1 engineering gate failed. Remote deployment is blocked.'}
...
if($pending.Count -ne 0){throw "R4.1 is code-only and requires no pending migrations. Found: $($pending -join ', ')"}
...
Write-Host 'Reading production counts before R4.1 code action (read-only, per-table)...' -ForegroundColor Cyan
$before=@(Get-RemoteCountSnapshot -Phase 'pre-r4-1' -Tables $AllTables);$beforeSig=Get-SnapshotSignature -Rows $before
Write-Host 'Running current live runtime smoke...' -ForegroundColor Cyan
.\scripts\RUNTIME_CHECK.ps1
if($LASTEXITCODE -ne 0){throw 'Current live runtime check failed before R4.1.'}

if(-not $DeployRemote){
    Write-Host ''
    Write-Host 'PRE-FLIGHT PASS - R4.1 CODE GATE.' -ForegroundColor Green
    Write-Host 'R4.1 containment browser gate, R4 regression, D1/R2/schema/runtime checks passed. No remote mutation was performed.' -ForegroundColor Green
    Write-Host 'After owner review run:' -ForegroundColor Yellow
    Write-Host '.\scripts\RUN_NX_COLLAB_1_UI_R4_1_RELEASE.ps1 -DeployRemote' -ForegroundColor White
    exit 0
}

Write-Host 'Deploying R4.1 code/assets correction...' -ForegroundColor Cyan
Invoke-Wrangler deploy
if($WranglerExitCode -ne 0){throw 'Cloudflare Worker deployment failed.'}

Write-Host 'Running public runtime checks after R4.1 deployment...' -ForegroundColor Cyan
.\scripts\RUNTIME_CHECK.ps1
if($LASTEXITCODE -ne 0){throw 'Runtime check failed after R4.1 deployment.'}
...
Write-Host 'Verifying migration state remains closed after R4.1 code-only deployment...' -ForegroundColor Cyan
$afterMigration=Get-MigrationState
if(@($afterMigration.Pending).Count -ne 0){throw "Unexpected pending migration appeared after R4.1 deployment: $(@($afterMigration.Pending) -join ', ')"}
Write-Host 'Re-reading production counts after R4.1 code-only deployment...' -ForegroundColor Cyan
$after=@(Get-RemoteCountSnapshot -Phase 'post-r4-1' -Tables $AllTables);$afterSig=Get-SnapshotSignature -Rows $after
if($beforeSig -ne $afterSig){throw "Production counts changed during R4.1 code-only deployment. Before=$beforeSig After=$afterSig"}
Write-Host 'PASS production counts unchanged across R4 code-only deployment.' -ForegroundColor Green
Invoke-ForeignKeyCheck -Phase 'post-r4-1'
$clientCodeMismatchesAfter=Get-RemoteCount -Sql "SELECT COUNT(*) AS rows_count FROM clients WHERE client_code IS NULL OR client_code <> ('CU-' || printf('%03d',id));" -Context 'post-r4-1 Client Code invariant'
if($clientCodeMismatchesAfter -ne 0){throw 'Client Code invariant changed during R4.1 deployment.'}
...
Write-Host ''
Write-Host 'NX-COLLAB-1 UI R4.1 REMOTE DEPLOY COMPLETE.' -ForegroundColor Green
Write-Host 'No migration, R2 creation, Secret change, or business-data seed was performed.' -ForegroundColor Green
Write-Host 'Final live acceptance: Admin ticket long-message containment at 100% zoom, composer visibility, plus retained R4 realtime/files/client-link/responsive acceptance.' -ForegroundColor Yellow
