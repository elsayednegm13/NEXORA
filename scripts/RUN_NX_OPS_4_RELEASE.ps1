...
$ManifestPath = Join-Path $PSScriptRoot 'NX_OPS_4_RELEASE_MANIFEST.json'
...
Write-Host 'NEXORA NX-OPS-4 CLEAN RELEASE RUNNER R2' -ForegroundColor Cyan
...
if($manifest.release -ne 'NX-OPS-4-R2-PORTAL-SUPPORT-FOUNDATION-CLEAN'){throw 'Unexpected release manifest identity.'}
...
$StageRoot=Join-Path $env:TEMP ("NEXORA_NX_OPS_4_R2_"+[guid]::NewGuid().ToString('N'))
...
try{
    ...
    Copy-Item -LiteralPath $ManifestPath -Destination (Join-Path $StageRoot 'scripts\NX_OPS_4_RELEASE_MANIFEST.json') -Force

    # Parse the staged release entry script with this machine's actual PowerShell parser before any Cloudflare call.
    $ApplyScript=Join-Path $StageRoot 'scripts\APPLY_NX_OPS_4.ps1'
    $parseTokens=$null
    $parseErrors=$null
    [void][System.Management.Automation.Language.Parser]::ParseFile($ApplyScript,[ref]$parseTokens,[ref]$parseErrors)
    if(@($parseErrors).Count -ne 0){
        $messages=@($parseErrors | ForEach-Object { $_.Message }) -join ' | '
        throw "PowerShell parser gate failed for APPLY_NX_OPS_4.ps1: $messages"
    }
    Write-Host 'PASS Windows PowerShell parser gate for staged APPLY_NX_OPS_4.ps1' -ForegroundColor Green

    $env:NEXORA_NODE_EXE=$NodeExe;$env:NEXORA_WRANGLER_JS=$WranglerJs
    Push-Location $StageRoot
    try{
        if($ApplyDataRemote){& .\scripts\APPLY_NX_OPS_4.ps1 -ApplyDataRemote}
        elseif($DeployRemote){& .\scripts\APPLY_NX_OPS_4.ps1 -DeployRemote}
        else{& .\scripts\APPLY_NX_OPS_4.ps1}
        if($LASTEXITCODE -ne 0){throw "NX-OPS-4 release execution failed with exit code $LASTEXITCODE."}
