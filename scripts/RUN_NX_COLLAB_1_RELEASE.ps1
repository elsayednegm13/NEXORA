...
if(@($remoteActions).Count -gt 1){throw 'R2 infrastructure, Data migration and Worker deployment are separate owner gates. Choose one action.'}

$SourceRoot = Split-Path -Parent $PSScriptRoot
$ManifestPath = Join-Path $PSScriptRoot 'NX_COLLAB_1_RELEASE_MANIFEST.json'
...
Write-Host 'NEXORA NX-COLLAB-1 CLEAN RELEASE RUNNER R3' -ForegroundColor Cyan
Write-Host 'Pinned Node/Wrangler, SHA-verified package, clean temporary stage, separate Infrastructure, Data and Code owner gates.' -ForegroundColor DarkGray

if(-not (Test-Path -LiteralPath $ManifestPath)){throw 'Release manifest is missing.'}
$manifest=Get-Content -Raw -Encoding UTF8 $ManifestPath | ConvertFrom-Json
if($manifest.release -ne 'NX-COLLAB-1-R3-D1-READ-RETRY-CORRECTION-CLEAN'){throw 'Unexpected release manifest identity.'}
...
$StageRoot=Join-Path $env:TEMP ("NEXORA_NX_COLLAB_1_R3_"+[guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $StageRoot -Force | Out-Null
$success=$false
try{
    Write-Host "Creating clean stage: $StageRoot" -ForegroundColor Cyan
    foreach($entry in $manifest.files){
    ...
    Copy-Item -LiteralPath $ManifestPath -Destination (Join-Path $StageRoot 'scripts\NX_COLLAB_1_RELEASE_MANIFEST.json') -Force

    # Parse the staged release entry script with this machine's actual PowerShell parser before any Cloudflare call.
    $ApplyScript=Join-Path $StageRoot 'scripts\APPLY_NX_COLLAB_1.ps1'
    $parseTokens=$null
    $parseErrors=$null
    [void][System.Management.Automation.Language.Parser]::ParseFile($ApplyScript,[ref]$parseTokens,[ref]$parseErrors)
    if(@($parseErrors).Count -ne 0){
        $messages=@($parseErrors | ForEach-Object { $_.Message }) -join ' | '
        throw "PowerShell parser gate failed for APPLY_NX_COLLAB_1.ps1: $messages"
    }
    Write-Host 'PASS Windows PowerShell parser gate for staged APPLY_NX_COLLAB_1.ps1' -ForegroundColor Green

    # Compile the exact staged Cloudflare deployment entrypoint before any remote action.
    # This specifically guards Durable Object export/binding mismatches.
    $DryRunDir=Join-Path $StageRoot '.wrangler-dry-run'
    Push-Location $StageRoot
    try{
        & $NodeExe $WranglerJs deploy --dry-run --outdir $DryRunDir
        if($LASTEXITCODE -ne 0){throw 'Wrangler deployment dry-run compile gate failed.'}
    }finally{Pop-Location;Remove-Item -LiteralPath $DryRunDir -Recurse -Force -ErrorAction SilentlyContinue}
    Write-Host 'PASS Wrangler deployment dry-run: Cloudflare entrypoint + Durable Object bindings compile' -ForegroundColor Green

    $env:NEXORA_NODE_EXE=$NodeExe;$env:NEXORA_WRANGLER_JS=$WranglerJs
    Push-Location $StageRoot
    try{
        if($CreateR2Remote){& .\scripts\APPLY_NX_COLLAB_1.ps1 -CreateR2Remote}
        elseif($ApplyDataRemote){& .\scripts\APPLY_NX_COLLAB_1.ps1 -ApplyDataRemote}
        elseif($DeployRemote){& .\scripts\APPLY_NX_COLLAB_1.ps1 -DeployRemote}
        else{& .\scripts\APPLY_NX_COLLAB_1.ps1}
        if($LASTEXITCODE -ne 0){throw "NX-COLLAB-1 release execution failed with exit code $LASTEXITCODE."}
    ...
    if($success){Remove-Item -LiteralPath $StageRoot -Recurse -Force -ErrorAction SilentlyContinue}
    else{Write-Host "Diagnostic stage preserved at: $StageRoot" -ForegroundColor Yellow}
}
