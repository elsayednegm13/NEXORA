...
$ManifestPath = Join-Path $PSScriptRoot 'NX_COLLAB_1_UI_R4_2_RELEASE_MANIFEST.json'
...
Write-Host 'NEXORA NX-COLLAB-1 UI R4.2 CLEAN RELEASE RUNNER' -ForegroundColor Cyan
Write-Host 'Code-only R4.2: SHA-verified clean stage, R4 regression + compact-density browser gate, Wrangler dry-run, then owner-controlled deploy.' -ForegroundColor DarkGray

if(-not (Test-Path -LiteralPath $ManifestPath)){throw 'R4 release manifest is missing.'}
$manifest=Get-Content -Raw -Encoding UTF8 $ManifestPath | ConvertFrom-Json
if($manifest.release -ne 'NX-COLLAB-1-UI-R4.2-COMPACT-DENSITY-PASS-CLEAN'){throw 'Unexpected R4.2 release manifest identity.'}
...
$StageRoot=Join-Path $env:TEMP ("NEXORA_NX_COLLAB_1_UI_R4_2_"+[guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $StageRoot -Force | Out-Null
$success=$false
try{
    Write-Host "Creating clean R4.2 stage: $StageRoot" -ForegroundColor Cyan
    foreach($entry in $manifest.files){
    ...
    Copy-Item -LiteralPath $ManifestPath -Destination (Join-Path $StageRoot 'scripts\NX_COLLAB_1_UI_R4_2_RELEASE_MANIFEST.json') -Force

    $ApplyScript=Join-Path $StageRoot 'scripts\APPLY_NX_COLLAB_1_UI_R4_2.ps1'
    $parseTokens=$null;$parseErrors=$null
    [void][System.Management.Automation.Language.Parser]::ParseFile($ApplyScript,[ref]$parseTokens,[ref]$parseErrors)
    if(@($parseErrors).Count -ne 0){$messages=@($parseErrors | ForEach-Object { $_.Message }) -join ' | ';throw "PowerShell parser gate failed for APPLY_NX_COLLAB_1_UI_R4_2.ps1: $messages"}
    Write-Host 'PASS Windows PowerShell parser gate for staged R4.2 apply script' -ForegroundColor Green

    $DryRunDir=Join-Path $StageRoot '.wrangler-dry-run'
    Push-Location $StageRoot
    try{& $NodeExe $WranglerJs deploy --dry-run --outdir $DryRunDir;if($LASTEXITCODE -ne 0){throw 'Wrangler R4.2 deployment dry-run compile gate failed.'}}
    finally{Pop-Location;Remove-Item -LiteralPath $DryRunDir -Recurse -Force -ErrorAction SilentlyContinue}
    Write-Host 'PASS Wrangler deployment dry-run: exact R4.2 Worker + Durable Object + D1 + R2 + Assets compile' -ForegroundColor Green

    $env:NEXORA_NODE_EXE=$NodeExe;$env:NEXORA_WRANGLER_JS=$WranglerJs
    Push-Location $StageRoot
    try{
        if($DeployRemote){& .\scripts\APPLY_NX_COLLAB_1_UI_R4_2.ps1 -DeployRemote}else{& .\scripts\APPLY_NX_COLLAB_1_UI_R4_2.ps1}
        if($LASTEXITCODE -ne 0){throw "R4.2 release execution failed with exit code $LASTEXITCODE."}
    ...
    if($success){Remove-Item -LiteralPath $StageRoot -Recurse -Force -ErrorAction SilentlyContinue}else{Write-Host "Diagnostic R4.2 stage preserved at: $StageRoot" -ForegroundColor Yellow}
}
