...
function projectHeader(p){const status=projectState(p),progress=progressValue(p);return `<section class="project-command-hero"><div class="project-command-main"><span class="project-command-avatar">${esc(initials(p.name))}</span><div class="project-command-copy"><span class="eyebrow">${esc(p.project_code||p.public_id)}</span><h2>${esc(p.name)}</h2><small>${esc(p.description||t('clientProjectWorkspace'))}</small></div></div><div class="project-command-metrics"><div class="project-command-metric"><span class="project-command-icon">${faIcon('building',{fallback:'•'})}</span><div><small>${esc(t('client'))}</small><strong>${esc(p.client_display_name||'—')}</strong><em>${esc(p.client_code||'')}</em></div></div><div class="project-command-metric"><span class="project-command-icon">${faIcon('calendar-check',{fallback:'•'})}</span><div><small>${esc(t('projectTarget'))}</small><strong>${esc(p.target_date||'—')}</strong><em>${esc(clientProjectPriorityLabel(p.priority))}</em></div></div><div class="project-command-progress" style="--project-progress:${progress}"><div><strong>${progress}%</strong></div><span>${esc(t('projectProgress'))}</span><small class="project-status project-status-${esc(status)}">${esc(clientProjectStatusLabel(status))}</small></div></div></section>`}
