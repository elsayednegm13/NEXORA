...
function moduleLabel(m){const labels=state.lang==='ar'?{overview:'نظرة عامة',execution:'التنفيذ',files:'الملفات'}:{overview:'Overview',execution:'Execution',files:'Files'};return labels[m.id]||t(m.labelKey)}
function moduleButton(m){return `<button type="button" data-project-module-jump="${m.id}" class="${m.id===active?'is-active':''}">${faIcon(m.icon,{fallback:'•'})}<span>${esc(moduleLabel(m))}</span>${m.id==='tickets'?'<b id="adminTicketNavCount">0</b>':''}</button>`}
...
function ticketList(){
  ...
  return list.map(x=>`<button class="admin-ticket-row ${selected?.id===x.id?'is-active':''}" data-open-ticket="${x.id}"><div class="admin-ticket-row-main"><small>${esc(x.ticket_code||x.public_id)}</small><strong>${esc(x.title)}</strong><span>${esc(ticketTypeLabel(x.type))}${x.updated_at?` · ${esc(x.updated_at)}`:''}</span></div><aside class="admin-ticket-row-state">${statusPill(x.status)}${priorityPill(x.priority)}</aside></button>`).join('')||`<div class="empty-inline">${esc(t('noTickets'))}</div>`;
}
...
function detail(){
  ...
  return `<div class="admin-ticket-detail-shell"><header class="admin-ticket-detail-head"><div class="admin-ticket-title-wrap"><button type="button" class="admin-ticket-mobile-back" data-ticket-mobile-back>${faIcon('arrow-right',{fallback:'←'})}<span>${esc(t('backToTickets'))}</span></button><div><div class="admin-ticket-title-line"><small>${esc(selected.ticket_code||selected.public_id)}</small><span class="admin-live-chip" data-realtime-chip>${faIcon('circle',{fallback:'•'})}<span data-realtime-label>${esc(t('realtimeConnecting'))}</span></span></div><h3 title="${esc(selected.title)}">${esc(selected.title)}</h3></div></div><div class="admin-ticket-control-group"><button class="admin-ticket-control-toggle" type="button" data-toggle-ticket-controls aria-expanded="false" aria-controls="adminTicketControls">${faIcon('sliders',{fallback:'≡'})}<span>${esc(t('ticketStatus'))}</span></button><div class="admin-ticket-controls" id="adminTicketControls"><label><span>${esc(t('ticketStatus'))}</span><select data-status class="select-input">${(selected.allowed_statuses||[selected.status]).map(x=>option(x,ticketStatusLabel(x),selected.status)).join('')}</select></label><label><span>${esc(t('ticketPriority'))}</span><select data-priority class="select-input">${(config?.ticket_priorities||[]).map(x=>option(x,ticketPriorityLabel(x),selected.priority)).join('')}</select></label><label><span>${esc(t('ticketAssignee'))}</span><select data-assignee class="select-input"><option value="">—</option>${(config?.members||[]).filter(x=>Number(x.is_active)!==0).map(m=>option(m.admin_user_id,m.name,selected.assigned_admin_id||'')).join('')}</select></label><button data-save-ticket class="ghost-btn compact-btn">${faIcon('floppy-disk',{fallback:'•'})}${esc(t('save'))}</button></div></div></header>${thread(selected)}${supportUi.newMessages?`<button type="button" class="admin-new-messages" data-new-messages>${faIcon('arrow-down',{fallback:'↓'})}<span>${esc(t('newMessages'))}</span></button>`:''}${closed?`<div class="admin-ticket-closed-note">${faIcon('lock',{fallback:'•'})}<div><strong>${esc(t('ticketClosed'))}</strong><small>${esc(t('ticketClosedAdminHint'))}</small></div></div>`:`<form data-message-form class="admin-ticket-composer">${pendingAttachments()}<div class="admin-composer-row"><input data-msg-files type="file" multiple hidden><button data-attach type="button" class="icon-btn" aria-label="${esc(t('ticketFiles'))}">${faIcon('paperclip',{fallback:'•'})}</button><textarea rows="1" name="body" maxlength="10000" aria-label="${esc(t('writeReply'))}" placeholder="${esc(t('writeReply'))}">${esc(supportUi.replyBody)}</textarea><select name="visibility" class="select-input" aria-label="${esc(t('ticketMessageVisibility'))}"><option value="client" ${supportUi.replyVisibility==='client'?'selected':''}>${esc(t('ticketVisibilityClient'))}</option><option value="internal" ${supportUi.replyVisibility==='internal'?'selected':''}>${esc(t('ticketVisibilityInternal'))}</option></select><button class="primary-btn compact-btn" type="submit">${faIcon('paper-plane',{fallback:'•'})}${esc(t('sendMessage'))}</button></div></form>`}</div>`;
}
...
function renderSupport(){
  ...
  r.innerHTML=`<div class="admin-support-workspace"><aside class="admin-ticket-sidebar"><div class="admin-ticket-list-head"><header class="admin-ticket-head"><div><span class="admin-workspace-icon">${faIcon('ticket',{fallback:'•'})}</span><div><strong>${esc(t('tickets'))}</strong><small>${supportUi.search||supportUi.status?`${filteredTickets().length} / `:''}${tickets.length} ${esc(t('tickets'))}</small></div></div><button data-new-ticket class="primary-btn compact-btn">${faIcon('plus',{fallback:'+'})}${esc(t('addTicket'))}</button></header><div class="admin-ticket-tools"><label class="admin-ticket-search">${faIcon('magnifying-glass',{fallback:'⌕'})}<input data-ticket-search value="${esc(supportUi.search)}" placeholder="${esc(t('searchTickets'))}"></label><select data-ticket-status-filter class="select-input"><option value="">${esc(t('allStatuses'))}</option>${(config?.ticket_statuses||[]).map(x=>option(x,ticketStatusLabel(x),supportUi.status)).join('')}</select></div></div><div class="admin-ticket-list">${ticketList()}</div></aside><section class="admin-ticket-detail">${detail()}</section></div>${newTicketDrawer()}`;
...
function bindSupport(){
  const r=$('#projectSupportRoot');if(!r)return;
  $('[data-toggle-ticket-controls]',r)?.addEventListener('click',e=>{const button=e.currentTarget;button.setAttribute('aria-expanded',String(button.getAttribute('aria-expanded')!=='true'))});
...
export function bindWorkspaceNav(){
  const activate=id=>{
    active=id;
    const label=$('#projectWorkspaceModuleLabel');if(label)label.textContent=moduleById(id)?moduleLabel(moduleById(id)):t('workspaceModules');
